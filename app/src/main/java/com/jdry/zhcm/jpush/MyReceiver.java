...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            ...
            if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
                ...
                AlarmBean alarmBean = JSON.parseObject(extraStr, AlarmBean.class);
                Intent i = new Intent(context, AlarmDetailActivity.class);
                i.putExtra("bean", alarmBean);
    ...
    //send msg to JDRYDYConstants
    private void processCustomMessage(Context context, Bundle bundle) {
        if (MainActivity.isForeground) {
            String message = bundle.getString(JPushInterface.EXTRA_MESSAGE);
            String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
            Intent msgIntent = new Intent(JDRYDYConstants.MESSAGE_RECEIVED_ACTION);
            msgIntent.putExtra(JDRYDYConstants.KEY_MESSAGE, message);
            if (!ExampleUtil.isEmpty(extras)) {
                try {
                    JSONObject extraJson = new JSONObject(extras);
                    if (extraJson.length() > 0) {
                        msgIntent.putExtra(JDRYDYConstants.KEY_EXTRAS, extras);
                    }
                } catch (JSONException e) {
